...
The constant-state prior ($\mathbf{F}=\mathbf{H}=\mathbf{I}_2$) is a
deliberate design choice suited for slowly varying IoT channels, yielding
the optimal MMSE channel estimate under Gaussian noise. The EKF framework
is selected over static methods (K-Means, GMM, LOF) for three reasons:
(i)~the time-varying Kalman gain $\mathbf{K}_t$ adaptively weights new
observations against the current uncertainty $\mathbf{P}_{t|t-1}$;
(ii)~the NIS statistic provides a chi-squared anomaly test grounded in
the filter's own uncertainty model; and (iii)~the Youden's J threshold
on the filtered RSSI estimate is data-adaptive across heterogeneous
IoT deployments. The EKF structure also accommodates future nonlinear
channel models without architectural redesign.
...
1 & VAE Semantic Compression
  & Provides a semantic state-space abstraction for high-dimensional,
    heterogeneous IoT telemetry via $\beta$-VAE ELBO to overcome the
    curse of dimensionality and significantly enhance downstream
    MARL sample efficiency
  & MSE $\leq 0.054$; dimensional reduction up to $5.25\times$
    ($84\!\to\!16$) while preserving discriminative channel features \\
\midrule
2 & EKF Digital Twin
  & Achieves \textit{joint} connection quality classification
    (adaptive Youden's~J threshold on $\hat{x}^{(1)}_{t|t}$)
    and real-time anomaly detection (bivariate NIS $\chi^2$ test)
    within a single, low-latency filter pass
  & Quality AUC up to $0.973$; Anomaly AUC up to $0.978$;
    outperforms static K-Means by up to $+80\%$ on LoRaWAN \\
\midrule
