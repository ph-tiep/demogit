...
computationally intractable and prone to gradient instability. We therefore
decompose $(\mathcal{P})$ into a \textit{sequential optimization paradigm}
with two discrete phases. In Phase~1, the semantic feature space is
optimized offline to bound the reconstruction error:
\begin{equation}
\phi^* = \arg\max_{\phi,\theta}\;\mathcal{L}_{\rm VAE}(\phi,\theta;\mathbf{X}),
\label{eq:vae_opt}
\end{equation}
subject to~\eqref{c1}. The encoder is then frozen and latent codes
$\hat{\mathbf{z}}_i(t)=\bm{\mu}_{\phi^*}(\mathbf{x}_i(t))$ are propagated
downstream. In Phase~2, the frozen representations inform the RL
optimization:
\begin{equation}
(\mathcal{P}_{\rm RL})\!:\;\max_{\{a_i\}}\;
  \mathbb{E}\!\left[\sum_{t=0}^{T}\sum_{i=1}^{N} U(t)
  \;\bigg|\;
  \hat{\mathbf{z}}_i(t)=\bm{\mu}_{\phi^*}(\mathbf{x}_i(t))\right]
\label{eq:rl_opt}
\end{equation}
subject to:
\begin{align}
&\text{(C.2)}\;\; \hat{q}_i(t) = \mathbf{1}\!\bigl[\widehat{\rm RSSI}_{i,t}\geq\tau^*\bigr],
...
decoupled formulation guarantees convergence and practical feasibility,
as detailed in the three-stage framework below.
...
state estimate. The state vector is $\mathbf{s}_i(t)=[\text{RSSI}_i(t),\,n^{\rm BS}_i(t)]^\top$
in dBm domain. The \textbf{nonlinear transition} model is:
\begin{align}
s^{(1)}_i(t+1) &= s^{(1)}_i(t)
...
The observation model is linear: $\mathbf{z}_i(t)=\mathbf{H}\mathbf{s}_i(t)+\mathbf{v}(t)$,
...
converted to dBm as $\widehat{\rm RSSI}_{t} = 10\log_{10}(\hat{s}^{(1)}_{t|t})$. The optimal
...
AUC = 0.911 — a relative gain of \textbf{+80.4\%}. For Indoor,
...
datasets, the framework demonstrates up to $+80.4\%$ quality AUC
improvement over K-Means on LoRaWAN, anomaly AUC of 0.987 on Indoor,
