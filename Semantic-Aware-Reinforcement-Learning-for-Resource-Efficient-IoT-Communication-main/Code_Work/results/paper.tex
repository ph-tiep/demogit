...
% ============================================================
\author{
Nguyen Thi Huyen Trang\textsuperscript{1}, 
Hoang Trong Binh\textsuperscript{1}, 
Nguyen Quoc Khanh\textsuperscript{1},\\ 
Ngo Van Khuong\textsuperscript{2}, 
Pham Van Tiep\textsuperscript{2}, and 
Dinh Van Linh\textsuperscript{2} \\[1em]
...
\textsuperscript{*}Corresponding author: vanlinh@actvn.edu.vn
}
% ============================================================
\begin{document}
\maketitle
...
and multi-agent coordination. In this study, we propose a three-stage framework combining:
...
Antwerp, LoRaWAN Italy, and Indoor) demonstrate that the proposed Digital Twin component achieves
an area under the ROC curve (AUC) for quality classification of up to 0.972, outperforming K-Means by up to
...
test~\cite{ref_anomaly_survey}, and the evaluation is confined to a
...
where  $\phi$ and $\theta$ represent the trainable weights of the encoder and decoder architectures, respectively. The initial term in Eq.~\eqref{eq:elbo} denotes the expected log-likelihood of the reconstruction, which evaluates the network'sability to recover the primary input $\mathbf{x}$ from the latent embedding $\mathbf{z}$.  In contrast, the latter term introduces the Kullback-Leibler (KL) divergence, $D_{\rm KL}$, serving as a regularization constraint. This mechanism minimizes the discrepancy between the approximate posterior distribution $q_\phi(\mathbf{z}|\mathbf{x})$ and the isotropic Gaussian prior $\mathcal{N}(\mathbf{0},\mathbf{I})$, thus fostering a smooth and cohesive latent manifold. Following the classical VAE paradigm~\cite{ref_vae}, the weighting factor $\beta$ is strictly set to $1$ throughout our evaluations.
...
In inference, the semantic latent code is the deterministic mean
$\hat{\mathbf{z}}=\bm{\mu}_\phi(\mathbf{x})\in\mathbb{R}^{d_z}$.
The compression ratios are $84\to16$ (Sigfox Antwerp) and $5\to3$
(LoRaWAN, Indoor WiFi).
...
The nonlinear transition model is:
\begin{align}
s^{(1)}_i(t+1) &= s^{(1)}_i(t)
...
quality classification. \textit{Rolling Z-Score} (window $W=20$,
$z_t=|x_t-\bar{x}|/\hat{\sigma}$) performs anomaly detection. Note that
K-Means/GMM cannot detect anomalies and Z-Score cannot classify
...
$\text{AUC}=0.9106$---a relative gain of +80.4\%. For Indoor,
...
These results confirm that the EKF Digital Twin, by exploiting temporal
state dynamics and multivariate anomaly scoring, outperforms static
baselines across both evaluation tasks and consistently generalizes to
heterogeneous IoT channel conditions.
