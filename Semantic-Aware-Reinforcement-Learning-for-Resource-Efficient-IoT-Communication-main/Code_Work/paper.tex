...
quality classification AUC up to 0.9725, outperforming K-Means by up to
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{framework_overview}
...
\mathcal{L}(\phi,\theta;\mathbf{x})=
  ...
  -D_{\rm KL}\!\left(q_\phi(\mathbf{z}|\mathbf{x})\,\|\,\mathcal{N}(\mathbf{0},\mathbf{I})\right).
\label{eq:elbo}
\end{equation}
Training uses the reparameterisation trick
...
The Digital Twin tracks the channel state $\mathbf{s}_i(t)$ in
(\ref{eq:channel_state}) using an EKF with a constant-state
prior~\cite{ref_ekf}. The state-space model is:
\begin{align}
...
and $\mathbf{R}=\sigma_R^2\mathbf{I}$. Although the model is linear, the
EKF framework is adopted to readily accommodate nonlinear extensions
(e.g., log-distance path-loss models for RSSI). The filter executes a
...
\midrule
Technology        & Sigfox  & LoRaWAN & Indoor   \\
...
feature-cluster structure), while DT-EKF reaches AUC = 0.9089 — a
...
\begin{figure*}[t]
  \centering
  \subfloat[Antwerp]{%
    \includegraphics[width=0.32\textwidth]{figures/roc_comparison_antwerp.png}}
  \hfill
  \subfloat[LoRaWAN]{%
    \includegraphics[width=0.32\textwidth]{figures/roc_comparison_lorawan.png}}
  \hfill
  \subfloat[Indoor]{%
    \includegraphics[width=0.32\textwidth]{figures/roc_comparison_indoor.png}}
